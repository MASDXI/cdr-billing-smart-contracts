...
abstract contract BillingManager is IBillingManager {
    ...
    mapping(bytes16 => mapping(uint256 => mapping(uint8 => Bill)))
        private _bills;
    ...
    function _currentBillPointer(
        bytes16 userId,
        uint256 blockNumber
    ) private view returns (Bill storage) {
        (uint256 cycle, uint8 slot) = _slidingWindows[userId].cycleAndSlot(
            _blockTime,
            blockNumber
        );
    ...
    function _caculateOutstandingBalanceOf(
        bytes16 userId,
        uint256 cycle,
        uint8 slot
        ...
        while (slot > 0) {
            uint slotOutstandingBalanceCache = _bills[userId][cycle][slot]
                .outstandingBalance;
    ...
    function _caculateOverdueBalanceOf(
        bytes16 userId,
        uint256 cycle,
        uint8 slot
    ) internal view returns (uint256 overdueBalance) {
        if (cycle > 0) {
            cycle--;
    ...
    function cdrOf(
        bytes16 userId,
        uint256 cycle
    ) public view virtual override hasInit(userId) returns (CDR[] memory) {
        ...
        for (uint8 slot = 0; slot < 30; slot++) {
            uint256[] memory templist = _bills[userId][cycle][slot]
                .list
                .toArray();
            for (uint256 i = 0; i < templist.length; i++) {
                // Change to start at 0
                cdrsCache[index] = _bills[userId][cycle][slot].CDRs[
                    templist[i]
                ];
                index++;
            }
    ...
    function cdrOf(
        ...
        for (uint256 i = 0; i < templist.length; i++) {
            // Iterate over the length of the templist
    ...
    function currentBillingCycleOf(
        bytes16 userId
    ) public view virtual override hasInit(userId) returns (uint256) {
        if (_pause[userId]) {
            (uint256 cycle, ) = _slidingWindows[userId].loadSnapShot();
            return cycle;
        } else {
            return
                _slidingWindows[userId].cycle(
                    _blockTime,
    ...
    function currentSizeOfCDRs(
        bytes16 userId
    ) public view virtual hasInit(userId) returns (uint256) {
    ...
    function outstandingBalanceOf(
        bytes16 userId
    ) public view override hasInit(userId) returns (uint256) {
        if (_pause[userId]) {
            (uint256 cycle, uint8 slot) = _slidingWindows[userId]
                .loadSnapShot();
    ...
    function addCDR(
        bytes16 userId,
        CDR memory record
    ) public virtual override whenNotPaused(userId) {
        if (!_initial[userId]) {
            _init(userId);
        }
        Bill storage bill = _currentBillPointer(userId, _blockNumberProvider());
        ...
        bill.outstandingBalance += record.cost;
        bill.list.add(index);
    ...
    function removeCDR(
        bytes16 userId,
        uint256 index
    ) public virtual override hasInit(userId) whenNotPaused(userId) {
        Bill storage bill = _currentBillPointer(userId, _blockNumberProvider());
        if (bill.list.contains(index)) {
            bill.outstandingBalance -= bill.CDRs[index].cost;
    ...
    function dischargeOutstandingBalanceOf(
        bytes16 userId,
        uint256 value
    ) public virtual override hasInit(userId) {
        Bill storage bill = _currentBillPointer(userId, _blockNumberProvider());
    ...
    function unpausedBilling(
        bytes16 userId
    ) public virtual override hasInit(userId) whenPaused(userId) {
        // @TODO load snapshot
        _pause[userId] = false;
